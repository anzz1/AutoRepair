...
function AR_Chat(text)
	DEFAULT_CHAT_FRAME:AddMessage(STATUS_COLOR .. "AutoRepair:|r " .. text);	
end
...
function AR_SlashHandler(msg)
   ...
   if (command == nil or command == "") then
	...
	AR_Chat("Plugin: ".. setEnabledString(AR_Save.enabled));
	AR_Chat("MinCost: ".. setAmountString(AR_Save.minCost));
	AR_Chat("Threshold: " .. setAmountString(AR_Save.costThreshold));
	AR_Chat("Ask Prompts: " .. setEnabledString(AR_Save.promptEnabled));
	AR_Chat("Verbose: " .. setEnabledString(AR_Save.verbose));
	AR_Chat("Skipping Inventory: " .. setEnabledString(AR_Save.skipInv));
	AR_Chat("Available Commands:");
	AR_Chat("/ar mincost <number>  -- " .. WHITE .."Sets the minimal amount you wish to ever auto repair for.  This is compared to the total repair costs.");
	...
	AR_Chat("/ar prompts -- " .. WHITE .."This toggles showing prompts.  If prompts are enabled you will be asked whether to repair your items.");
	AR_Chat("/ar enable -- " .. WHITE .."This turns AutoRepair on and off.");
	AR_Chat("/ar verbose -- " .. WHITE .. "This toggles showing repair costs in the chat area after repair has been done.");
   ...
   elseif (command == 'prompts')	then AR_PromptHandler();
...
function AR_VerboseHandler()
	if (AR_Save.verbose) then
		...
		AR_Chat("Verbose mode has been disabled.  Type /ar verbose to enable it.");
	else
		AR_Save.verbose = true;
		AR_Chat("Verbose mode has been enabled.  type /ar verbose to disable it.");
	end
end
...
function AR_PromptHandler()
	if (AR_Save.promptEnabled) then
		...
		AR_Chat("Prompts have been disabled.  Type /ar prompts to enable them.");
		AR_Chat("Your equipment will be always automatically repaired.");
	else
		AR_Save.promptEnabled = true;
		AR_Chat("Prompts have been enabled.  type /ar prompts to disable them.");
		AR_Chat("You will be asked whether to repair your equipment when price goes over your cost threshold. Threshold: "..setAmountString(AR_Save.costThreshold)..".");
	end
end
...
function AR_EnableHandler()
	if (AR_Save.enabled) then
		...
		AR_Chat("AutoRepair has been disabled.  Type /ar enable to turn it back on.");
	else
		AR_Save.enabled= true;
		AR_Chat("AutoRepair has been enabled.  type /ar enable to turn it off.");
	end
end
...
function AR_MinCostHandler(option)
   ...
   if(option == nil or option == "") then
		AR_Chat("Min. Repair Cost: "..setAmountString(AR_Save.minCost)..".");
	...
	num = tonumber(option);
	if (num < 0) then
		AR_Chat("Min. Repair Cost Set Error.");		
		...
		AR_Save.minCost = num;
		AR_Chat("Min. Repair Cost Set To: "..setAmountString(AR_Save.minCost)..".");		
	end
...
function AR_ThresholdHandler(option)
   ...
   if(option == nil or option == "") then
		AR_Chat("Threshold Amount: "..setAmountString(AR_Save.costThreshold)..".");
	...
	if (num < 0) then
		...
		AR_Chat("Threshold Amount Set To: "..setAmountString(AR_Save.costThreshold)..".");		
...
function AR_ShowInvPrompt(cost) 
	...
	StaticPopupDialogs["REPAIR_INV_BIND"] = {
		text = TEXT(STATUS_COLOR.."AutoRepair v"..AR_CurrentVersion.."\n\n\n|rIt will cost "..AR_CostText.." to fix the items in your inventory.\n\nDo You Wish To Repair Your Items?\n\n"),
		button1 = TEXT(OKAY),
		button2 = TEXT(CANCEL),
		OnAccept = function()
			AR_RepairInventory();
...
function AR_RepairEquipment()
	...
	if (AR_Save.verbose) then
		...
		AR_Chat("Your repair bill for your equipment was: " .. setAmountString(cost) .. ".");
...
function AR_RepairInventory()
	...
	if (AR_Save.verbose) then
		...
		AR_Chat("Your repair bill for your inventory was: " .. setAmountString(cost) .. ".");
